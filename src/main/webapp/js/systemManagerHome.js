...
var newRestURL = "restaurantController/createNewRestaurant"
		...
		var apc = $.ajax({
			...
			success : function(data) {
				...
				if (data == "da") {
					...
					// $('#errorMessage').val("You are not a MAIN System
					// manager");
					// $('#errorMessage').val("You are not a MAIN System
					// manager");
				}
	...
	$
			.each(
					...
					function(index, systemManager) {
						...
						tr
								.append('<td>'
										+ systemManager.system_manager_nick_id
										+ '</td>'
										+ '<td>'
										+ systemManager.manager_email
										+ '</td>'
										+ '<td>'
										+ systemManager.manager_name
										+ '</td>'
										+ '<td>'
										+ systemManager.manager_last_name
										+ '</td><td>'
										+ '<form class="delSM'
										+ brojac
										+ '" > '
...
/*
 * //-----------LOGOVANJE $(document).on('submit', '.loginForm', function(e) {
 * e.preventDefault(); console.log("registration begin"); var email =
 * $(this).find("input[name=emailLogin]").val(); var password =
 * $(this).find("input[name=passwordLogin]").val(); $.ajax({ type : 'POST', url :
 * loginURL, contentType : 'application/json', dataType : "text", data :
 * formToJSONLogin(email, password), success: function(data) { if(data ==
 * "Guest") { window.location.href = "http://localhost:8080/GuestHome.html"; }
 * else { window.location.href = "http://localhost:8080/"; } } }); });
 * 
 * function formToJSONLogin(email, password) { return JSON.stringify({ "email" :
